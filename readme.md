# 📘 Backend Core Concepts & Authentication Notes
...
When a client sends a request (e.g., using `fetch` with method POST), the data reaches the server in a **streamed/raw format** and is not directly readable.
This middleware parses the incoming JSON data and makes it available in `req.body`.
